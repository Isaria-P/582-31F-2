// LIGHT_DOM --> regulare Page Dome components
...
// uses shadow dome when we want a reusable widget.

// use SHADOW-DOM ---> the inner DOM attached to a custom element.

class HelloShadow extends HTMLElement {
  connectedCallback() {
    // creating shadow root for the element
    const shadow = this.attachShadow({ mode: "open" });
    // Mode: "open" means the shadow root can still be accessed in JS if needed.
    
    // render the component's inside internal content inside the shadow root and
...
/* 
we are still rendering the element, but its internal structure belongs to 
its own Dom layer.=>  const shadow = this.attachShadow({ mode: "open" });
*/
customElements.define("hello-shadow", HelloShadow);

// Without SHADOW DOM:
//  may change component styles may like into the page.
// - outside page styles may change component unexpected ly
// - component structure becomes more self-contained!

/* with SHADOW_DOM:
  - internal styles stay scopeed to the component 
  - component stucture becomes more self-contained!
*/

class FanceyCard extends HTMLElement {
  connectedCallback() {
    const shadow = this.attachShadow({ mode: "open" });
...
customElements.define("fancy-card", FanceyCard)
